...
public class RidePoolApp {
    ...
    private static void addSampleData() {
        availableRides.add(new CarpoolRide(nextRideId++, "Ali", "UCP", "Johar Town", 4));
        availableRides.add(new CarpoolRide(nextRideId++, "Ahmed", "Model Town", "Liberty Market", 3));
        availableRides.add(new CarpoolRide(nextRideId++, "Sara", "Wapda Town", "UCP", 2));
    }
    ...
    private static void addAvailableRide() {
        ...
        CarpoolRide ride = new CarpoolRide(nextRideId++, driverName, pickupArea, destination, seats);
    ...
    private static void submitRiderRequest() {
        ...
        // Queue stores requests in FIFO order
        requestQueue.add(request);

        // ArrayList stores complete request history
    ...
    private static void processNextRequest() {
        ...
        // Data Structure 3: Recursion
        CarpoolRide matchedRide = findMatchingRideRecursive(availableRides, request, 0);

        if (matchedRide != null) {
            matchedRide.addPassenger(request.riderName, request.seatsNeeded);
            request.status = "Matched with Ride ID " + matchedRide.rideId;

            System.out.println("\nMatch found!");
            System.out.println("Rider assigned to Ride ID: " + matchedRide.rideId);
            System.out.println("Driver Name: " + matchedRide.driverName);
        } else {
            request.status = "Waiting - No matching ride found";

            // Request is added back to the queue because no ride was found
            requestQueue.add(request);

            System.out.println("\nNo matching ride found.");
            System.out.println("Request has been placed back in the waiting queue.");
        }
    }
    ...
    private static void searchRiderRequest() {
        ...
        String name = readNonEmptyString("Enter rider name to search: ");

        boolean found = searchRiderRecursive(allRequests, name, 0);

        if (!found) {
            System.out.println("\nNo request found for rider name: " + name);
        }
    }
    ...
    /*
     * Recursive function to find a matching ride.
     * It checks one ride at a time.
     */
    private static CarpoolRide findMatchingRideRecursive(
        ...
        boolean pickupMatches = isSimilarLocation(currentRide.pickupArea, request.pickup);
        boolean destinationMatches = isSimilarLocation(currentRide.destination, request.destination);
        boolean seatsAvailable = currentRide.availableSeats >= request.seatsNeeded;

        if (pickupMatches && destinationMatches && seatsAvailable) {
            return currentRide;
        }

        return findMatchingRideRecursive(rides, request, index + 1);
    }

    /*
     * Recursive function to search rider requests by name.
     */
    private static boolean searchRiderRecursive(
        ...
        RiderRequest currentRequest = requests.get(index);

        boolean foundInRemainingList = searchRiderRecursive(requests, name, index + 1);

        if (currentRequest.riderName.equalsIgnoreCase(name)) {
            System.out.println("\nRequest found:");
            currentRequest.display();
            return true;
        }

        return foundInRemainingList;
    }

    /*
     * Recursive function to display all requests.
     */
    private static void displayRequestsRecursive(
        ...
        displayRequestsRecursive(requests, index + 1);
    }

    private static boolean isSimilarLocation(String locationOne, String locationTwo) {
        String first = locationOne.trim().toLowerCase();
        String second = locationTwo.trim().toLowerCase();
    ...
    private static int readPositiveOrZeroInt(String message) {
        int number;
    ...
    static class CarpoolRide {
        ...
        // ArrayList stores passengers assigned to this ride
        ArrayList<String> passengers;

        CarpoolRide(int rideId, String driverName, String pickupArea, String destination, int totalSeats) {
            this.rideId = rideId;
        ...
        void addPassenger(String passengerName, int seatsBooked) {
    ...
    static class RiderRequest {
        ...
        RiderRequest(int requestId, String riderName, String pickup, String destination, int seatsNeeded) {
